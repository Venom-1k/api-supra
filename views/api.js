...
try {
var zahirr = db.get("Venom");
} catch (e) {
	console.log('')
}
var fs = require('fs');
var creatorList = ['@Venom'];
...
loghandler = {
    notparam: {
        ...
        getApikey: 'Não tem apikey? Chame e compre sua chave por apenas 10 R$ wa.me/559784388524'
    },
    notkey: {
        status: false,
        criador: `${creator}`,
    ...
    invalidKey: {
        ...
        criador: `${creator}`,
        code: 406,
        mensagem: 'apikey invalida, Não tem apikey? Chame e compre sua chave por apenas 10 R$ wa.me/559784388524'
    },
    invalidlink: {
        status: false,
        criador: `${creator}`,
...
const listkey = ["Venom", "evolution", "kk"];
var len = 15
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'Venom') return res.json(loghandler.invalidKey)
...
if(!Apikey) return res.json(loghandler.notparam)
	if(listkey.includes(Apikey)){    
   hasil = 'https://lolizit-api.herokuapp.com/api/canvas/bemvindo?nome=Venom&nomegp=CABAR%C3%89&membros=50&perfil='+ text +'&fotogp=' + text + '&fundo=https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSxANrjm0j_uzUclnOAlHjtgroMtblwSPChJg&usqp=CAU&apikey=Venom'
	  data = await fetch(hasil).then(v => v.buffer())   
...
router.get('/base', async (req, res, next) => {
	var type = req.query.type,
		...
		if (apikeyInput != 'Venom') return res.json(loghandler.invalidKey)
